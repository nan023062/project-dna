# Agentic OS - Cursor 接入指南

Agentic OS 通过 MCP (Model Context Protocol) 与 Cursor 集成，让 AI 助手具备项目级上下文与记忆能力。
...
### 1. 复制 Skill 目录
将 `tools/dna-for-cursor` 复制到您项目的 `.cursor/skills/agentic-os` 目录下。

### 2. 修改配置文件
打开 `.cursor/skills/agentic-os/config.json`，按您的 **App 地址** 修改：
...
```json
{
  "app": {
    ...
    "hook": {
      ...
      "ruleFileName": "agentic-os-mcp-hook.mdc",
...
### 2. 引入工作流规则
建议将 `templates/rules/agentic-os-mcp-hook.mdc` 复制到项目 `.cursor/rules/` 目录。

---

## 验证连接

1. 重启 Cursor 后，在 Settings -> Features -> MCP 查看 `agentic-os` 是否为 `Connected`。
