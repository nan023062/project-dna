...
1. **检查配置**：确保 `config.json` 中的 `port` 已配置。
2. **关闭旧进程**：
   ```bash
   taskkill /IM dna.exe /F 2>nul
   # 或 macOS/Linux: pkill -f "dna.dll" || true
   ```
3. **编译并启动**：
   ```bash
   cd project-dna/src
   dotnet build Server/server.csproj -c Debug -v q
   ```
   如果编译成功，执行启动（Windows 示例）：
   ```bash
   .cursor\skills\start-dna-server\start-dna.cmd
   ```
   *(注：如果后续将启动脚本也移入本 Skill，可改为执行 `.\.cursor\skills\project-dna\scripts\start-server.ps1`)*
4. **验证结果**：
   等待 3 秒后执行 `curl http://localhost:<config.port>/api/status`。
