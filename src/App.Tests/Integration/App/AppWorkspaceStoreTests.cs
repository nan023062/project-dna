...
public sealed class AppWorkspaceStoreTests : IDisposable
{
    private readonly string _workspaceRoot = Path.Combine(Path.GetTempPath(), "dna-app-workspaces", Guid.NewGuid().ToString("N"));
    private readonly string _configPath;

    public AppWorkspaceStoreTests()
    {
        Directory.CreateDirectory(_workspaceRoot);
        _configPath = Path.Combine(_workspaceRoot, "app-workspaces.json");
    }
    ...
    public void Store_ShouldPersistCreatedWorkspace_AndSwitchCurrent()
    {
        ...
        var created = store.CreateWorkspace(new AppWorkspaceUpsertRequest
        {
            Name = "Remote Team",
            Mode = "team",
    ...
    {
        var store = CreateStore("http://localhost:5051");
        var remote = store.CreateWorkspace(new AppWorkspaceUpsertRequest
        {
            Name = "Remote Team",
            Mode = "team",
            ServerBaseUrl = "https://dna.example.com",
    ...
    private AppWorkspaceStore CreateStore(string serverBaseUrl)
    {
