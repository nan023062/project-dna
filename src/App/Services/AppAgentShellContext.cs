...
public sealed class AppAgentShellContext(
    DnaServerApi api,
    AppWorkspaceStore workspaceStore,
    AppRuntimeOptions options) : IAgentShellContext
{
    public string HostKind => "app";
    ...
    public async Task<string> GenerateReplyAsync(AgentChatRequest request, CancellationToken cancellationToken = default)
    {
        ...
        sb.AppendLine("后续会把更完整的 agent 编排能力接到独立共享库里，但不是让 App 依赖远端 Server。");
        sb.AppendLine();
        ...
        if (!string.IsNullOrWhiteSpace(latestUserMessage))
        {
            ...
            if (ContainsAny(latestUserMessage, "编排", "agent", "执行", "workflow", "plan"))
            {
                ...
                sb.AppendLine("后续接入统一 agent 执行库后，App 会作为独立宿主运行自己的编排任务。");
                sb.AppendLine();
            }
        }
