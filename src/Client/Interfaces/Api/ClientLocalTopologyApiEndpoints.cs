...
public static class ClientLocalTopologyApiEndpoints
{
    ...
    public static void MapClientLocalTopologyApiEndpoints(this IEndpointRouteBuilder app)
    {
        var api = app.MapGroup("/api");

        api.MapGet("/topology", ([FromServices] IGraphEngine graph, [FromServices] ClientRuntimeOptions runtime) =>
        {
    ...
    private static List<RelationEdgeDto> BuildContainmentEdges(List<ModuleDto> modules, Dictionary<string, string> disciplineNames)
    {
    ...
    private static void BuildExplicitContainmentEdges(List<ModuleDto> modules, List<RelationEdgeDto> edges, HashSet<string> seen)
    {
        ...
        foreach (var child in modules)
        {
            ...
            TryAddContainmentEdge(parent.Name, child.Name, "composition", false, edges, seen);
        }
        ...
        string? crossWorkName)
    {
        ...
        if (!string.IsNullOrWhiteSpace(crossWorkId) &&
            !(edge.CrossWorkIds?.Contains(crossWorkId, StringComparer.OrdinalIgnoreCase) ?? false))
        {
            edge.CrossWorkIds ??= [];
        ...
        if (!string.IsNullOrWhiteSpace(crossWorkName) &&
            !(edge.CrossWorkNames?.Contains(crossWorkName, StringComparer.OrdinalIgnoreCase) ?? false))
        {
            edge.CrossWorkNames ??= [];
