...
public sealed class ClientPipelineStore
{
    ...
    private ClientExecutionPipelineConfig? _cachedConfig;
    private PipelineRunResult? _latestRun;

    public ClientPipelineStore()
    {
        var home = Environment.GetFolderPath(Environment.SpecialFolder.UserProfile);
        var dir = Path.Combine(home, ".dna");
        Directory.CreateDirectory(dir);

        _configPath = Path.Combine(dir, "client-pipeline.json");
        _latestRunPath = Path.Combine(dir, "client-pipeline-last-run.json");
    }

    public ClientExecutionPipelineConfig GetConfig()
    {
        lock (_sync)
        {
            ...
            try
            {
                ...
                _cachedConfig = JsonSerializer.Deserialize<ClientExecutionPipelineConfig>(json, JsonOptions)
                                ?? BuildDefaultConfig();
            }
            catch
            {
    ...
    public ClientExecutionPipelineConfig UpdateConfig(ClientExecutionPipelineConfig config)
    {
        lock (_sync)
        {
            Normalize(config);
    ...
    private void SaveConfigLocked(ClientExecutionPipelineConfig config)
    {
        var json = JsonSerializer.Serialize(config, JsonOptions);
        File.WriteAllText(_configPath, json);
    }

    private static ClientExecutionPipelineConfig BuildDefaultConfig()
    {
        var config = new ClientExecutionPipelineConfig();
        Normalize(config);
        return config;
    }

    private static void Normalize(ClientExecutionPipelineConfig config)
    {
