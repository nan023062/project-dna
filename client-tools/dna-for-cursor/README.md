...
## 目录
- [前提条件](#前提条件)
- [方式一：使用自动化脚本接入（推荐）](#方式一使用自动化脚本接入推荐)
...
我们在本目录下提供了一套自动化配置脚本，可以一键完成 MCP 配置和规则模板的下发。

### 1. 复制配置目录
将本目录（`client-tools/dna-for-cursor`）复制到您当前开发项目的任意位置，例如 `.cursor/dna-setup`。

### 2. 修改配置文件
打开 `config.json`，根据您的 DNA Server 实际地址修改配置：
```json
{
  "serverIp": "127.0.0.1",
  "port": 5051,
