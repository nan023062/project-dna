...
### 1. 复制 Skill 目录
将本目录（`client-tools/dna-for-cursor`）复制到您当前开发项目的 `.cursor/skills/project-dna` 目录下。
...
Project DNA Server 自带了一个可视化的 Web Dashboard，您可以通过浏览器直接访问它来查看项目知识图谱和记忆：

1. 打开浏览器。
2. 访问您在 `config.json` 中配置的地址，例如：`http://127.0.0.1:5051`。
3. 您可以在 Dashboard 中直观地浏览模块依赖、编辑记忆节点、触发知识压缩等。
